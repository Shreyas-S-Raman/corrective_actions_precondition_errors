...
class RecordingInterface(object):
    ...
    def __init__(self, robot, download_filepath, client_metadata):
        ...
        self._command_dictionary = {
            ...
            '5': self._create_named_waypoint,
            '6': self._download_full_graph,
            '7': self._list_graph_waypoint_and_edge_ids,
            '8': self._create_new_edge,
            '9': self._create_loop,
            '10': self._auto_close_loops_prompt,
    ...
    def _create_named_waypoint(self, *args):
        """Create waypoint with specified name at robot's current location"""
        print(args[0][0])
        resp = self._recording_client.create_waypoint(waypoint_name=args[0][0])

        if resp.status == recording_pb2.CreateWaypointResponse.STATUS_OK:
            print('Created waypoint for {}'.format(args[0]))
        else:
            print('Could not create named waypoint')
