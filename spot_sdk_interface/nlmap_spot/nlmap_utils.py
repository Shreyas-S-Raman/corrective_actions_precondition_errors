...
def get_best_clip_vild_dirs(model,preprocess,img_names,img_dir_path,use_softmax=False,cache_images=True,cache_text=True,cache_path="./cache/",img_dir_name=None,category_names=None,headless=False):
	...
	priority_queue_combined = defaultdict(lambda: PriorityQueue())
	...
	for img_name in tqdm(img_names):
		...
		if cache_images and cache_img_exists:
			...
			print("IN ELSE CONDITION FOR NLMAP UTILS")
		...
		for anno_idx in indices[0:int(n_boxes)]:
			...
			for idx, category_name in enumerate(category_names):
				new_item = (-scores[idx], (img_name,anno_idx,crop,ymin[anno_idx],xmin[anno_idx],ymax[anno_idx],xmax[anno_idx], segmentations[anno_idx]))
			...
			for idx, category_name in enumerate(category_names):
				priority_queue_clip_dir[category_name].put((-clip_scores[0][idx], (img_name,anno_idx,crop,ymin[anno_idx],xmin[anno_idx],ymax[anno_idx],xmax[anno_idx],segmentations[anno_idx]))) #TODO: make this an object to more interpretable

			#populate the combined queue: takes the max score between CLIP and ViLD
			for idx, category_name in enumerate(category_names):
				max_score = max(scores[idx], clip_scores[0][idx])

				priority_queue_combined[category_name].put((-max_score, (img_name, anno_idx, crop, ymin[anno_idx], xmin[anno_idx], ymax[anno_idx], xmax[anno_idx], segmentations[anno_idx], abs(max_score))))
	...
	return(priority_queue_vild_dir, priority_queue_clip_dir, priority_queue_combined)