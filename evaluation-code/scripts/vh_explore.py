...
scene_graph_to_load = None

class VirtualHomeSimulator():
    
    def __init__(self, version=1, scene=0, new_scene_graph=None):
        if version == 1:  
        ...
        # -- there are 7 scenes available in VirtualHome; by default, we will use the first one: = scene
    ...
    def _start_simulator(self):
        ...
        if self.scene_graph:
            # -- load a scene graph from Wenlong's dataset:
            self.comm.expand_scene(json.load(open(self.scene_graph,'r')))
    ...
    def _run_script(self, script):
        ...
        while True:
            # is_success, message = self.comm.render_script(
            #         script=script, 
            #         recording=True, 
            #         frame_rate=fps,
            #         output_folder=output_frames_dir)

            is_success, message = self.comm.render_script(script=script, recording=True, find_solution = True)
...
if __name__ == '__main__':
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], "env:scr:scg", ['scene=', 'script=', 'scene_graph='] )
        ...
        for opt, arg in opts:
         ...
         # -- add the character to
            elif opt in ['-scg', '--scene_graph']:
                scene_graph_to_load = str(arg)
    ...
    vh_comm = VirtualHomeSimulator(scene=scene_number,new_scene_graph=scene_graph_to_load)
    ...
    print('Final script: ', script_to_execute)
