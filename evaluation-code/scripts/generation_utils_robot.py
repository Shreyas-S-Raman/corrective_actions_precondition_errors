...
import spacy
import random
from actions import fill_action_template
...
def robot_program_lines2program_english(robot_program_lines):

    program_english = ''

    for l, line in enumerate(robot_program_lines):

        line = [x.replace('_',' ') for x in line.split(' ')]
        
        action_str = fill_action_template(line)
...
def construct_example_robot(example_path, add_desc=False):

    example = load_txt(example_path)
    full_program_lines = example.strip().split('\n')
    title = full_program_lines[0]
    
    program_english = robot_program_lines2program_english(full_program_lines[3:])
...
def iterative_api_request(example, task_prompt, api_params, sentence_model, action_list_embedding, device, action_list, max_iters=1000, max_steps=20, verbose=False, cutoff_threshold=-100, beta=0.5, percent_terminate=0.6, engine='davinci-codex', translated_condition=False, step_by_step = False, use_robot=False):
    ...
    while curr_step < max_steps:
        ...
        if translated_condition:
            ...
            best_curr = [x.replace('_',' ') for x in best_curr.split(' ')]
            best_curr = fill_action_template(best_curr)
...
def online_api_request_one_error_full(example, task_prompt, api_params, sentence_model, action_list_embedding, device, action_list, raw_lm, scene_path, scene_num, prompt_args, max_iters=1000, max_steps=20, verbose=False, cutoff_threshold=-100, beta=0.5, percent_terminate=0.6, engine='davinci-codex', translated_condition=False, step_by_step = False, add_executable_mask = False, robot_hostname='138.16.161.22'):
    ...
    def _generate_action(full_text, default_params, executed):
        ...
        for idx in highest_ids:
            # if len(curr_generated[idx]) > 0 and (curr_step == 0 or int(re.search(r'\d+', curr_generated[idx]).group()) == curr_step+1 or not executed):
    ...
    scene_environment = SlangGymEnvironment(hostname=robot_hostname, with_robot=False, scene_num=scene_num, task = task_prompt, scene_path = scene_path)
    ...
    while curr_step < max_steps and total_steps < max_steps*2:
        ...
        best_curr, generated_action, translated_action, nogen_terminate, score_terminate, error_message = _generate_action( prompt_generator.change_context(ongoing_text if curr_step == 0 else ongoing_text + 'Step {}'.format(curr_step+1), executed), default_params, executed)
        ...
        program_lines = remove_same_consecutive(ongoing_translated_actions)
        ...
        if len(program_lines) == 0:
            ...
            error_prompt =  prompt_generator.generate_prompt_robot('empty_program', None, None, total_steps, best_curr.replace('Step {}:'.format(curr_step+1), '').strip(), program_lines[-1])

            full_text += error_prompt + '\n' if translated_condition else '{}\nStep {}:'.format(error_prompt, curr_step+1)
            ongoing_text += error_prompt + '\n' if translated_condition else '{}\nStep {}:'.format(error_prompt, curr_step+1)
        ...
        if precondition_error:
            ...
            precond_error = 'Script Fail: {}'.format(error_msg)
            all_errors.append(precond_error)

            error_prompt = prompt_generator.generate_prompt_robot('check_script', obj, skill, total_steps, best_curr.replace('Step {}:'.format(curr_step+1), '').strip(), program_lines[-1], error_params)
        ...
        #take a single step/action in the Slang scene
        success, steps, object_states, room_state, agent_state = scene_environment.step(best_curr, translated_action)

        #TODO: check if replanning can be done after step execution with robot fails in real-time (beyond precondition)
        #if not success:
        #    pass
    ...
    info = { 'parsed_program': None if total_steps==0 else '\n'.join            (program_lines).strip(), 
        ...
        'percent_executed': 1.0 if (executed and total_steps>0) else 0.0 ,'scene_path': scene_num,
        'init_graph_dict': scene_environment.get_current_state(), 'modified_program': None,
        'execution_error': precondition_error, 'precond_error': precondition_error, 'parsing_error':None,
        'empty_program_error':empty_program_error, 'total_steps': total_steps, 'final_steps': curr_step, 'num_replans': total_steps - curr_step,  'no_gen_error':no_gen_error, 'score_error':score_error,  'all_errors': '\n'.join(all_errors), 'avg_affordance':None, 'step_probabilities':None, 'closest_probabilities':None}
...
def resampling_api_request_full(example, task_prompt, api_params, sentence_model, action_list_embedding, device, action_list, raw_lm, scene_path, scene_num, max_iters=1000, max_steps=20, verbose=False, cutoff_threshold=-100, beta=0.5, percent_terminate=0.6, engine='davinci-codex', translated_condition=False, step_by_step = False, use_robot=False, robot_hostname='138.16.161.22'):
    ...
    def _generate_action(full_text, default_params):
        ...
        def _get_curr(curr_translated, curr_generated, translated_condition, idx):
            if translated_condition:
                ...
                curr = [x.replace('_',' ') for x in curr.split(' ')]
                curr = fill_action_template(curr)
    ...
    info = { 'parsed_program': None if total_steps==0 else '\n'.join(program_lines).strip(), 'executed': False if total_steps==0 else executed, 'percent_executed': 1.0 if (executed and total_steps>0) else  0.0, 'percent_executed_inloop': 0.0 if total_steps==0 else curr_step/total_steps, 'scene_path': scene_num,
        'init_graph_dict': scene_environment.get_current_state(), 'modified_program': None,
        'execution_error': precondition_error, 'precond_error': precondition_error, 'parsing_error':None,
        'empty_program_error':empty_program_error, 'total_steps': total_steps, 'final_steps': curr_step, 'num_replans': total_steps - curr_step, 'no_gen_error':no_gen_error, 'score_error':score_error,  'all_errors': '\n'.join(all_errors), 'avg_affordance': None, 'step_probabilities':None, 'closest_probabilities':None}
...
def incontext_learned_api_request_one_error_full(example, task_prompt, api_params, sentence_model, action_list_embedding, corrections_example_embedding, device, action_list, corrections_example_paths, raw_lm, scene_path, scene_num, prompt_args, max_iters=1000, max_steps=20, verbose=False, cutoff_threshold=-100, beta=0.5, percent_terminate=0.6, engine='davinci-codex', translated_condition=False, step_by_step = False, add_executable_mask = False, use_robot=False, robot_hostname='138.16.161.22'):
    ...
    info = { 'parsed_program': None if total_steps==0 else '\n'.join            (program_lines).strip(), 
        ...
        'percent_executed': 1.0 if (executed and total_steps>0) else 0.0 ,'scene_path': scene_num,
        'init_graph_dict': scene_environment.get_current_state(), 'modified_program': None,
        'execution_error': precondition_error, 'precond_error': precondition_error, 'parsing_error':parsing_error,
        'empty_program_error':empty_program_error, 'total_steps': total_steps, 'final_steps': curr_step, 'avg_affordance':None, 'num_replans': total_steps - curr_step,  'no_gen_error':no_gen_error, 'score_error':score_error,  'all_errors': '\n'.join(all_errors), 'step_probabilities':None, 'avg_affordance':None, 'closest_probabilities':None}
...
def predicted_learned_api_request_one_error_full(example, task_prompt, api_params, api_generation_params, sentence_model, action_list_embedding, corrections_example_embedding, device, action_list, corrections_example_paths, raw_lm, scene_path, scene_num, prompt_args, max_iters=1000, max_steps=20, verbose=False, cutoff_threshold=-100, beta=0.5, percent_terminate=0.6, engine='davinci-codex', translated_condition=False, step_by_step = False, add_executable_mask = False, use_robot=False, robot_hostname='138.16.161.22'):
    ...
    scene_environment = SceneGymAllSteps(scene_path, scene_num, task_prompt)  if not use_robot else SPOTAction(hostname=robot_hostname)
    ...
    def _generate_action(full_text, translated_actions, admissible_actions, default_params):
        ...
        for i, action in enumerate(filtered_admissible_actions):
            ...
            if action != 'done':
                ...
                program_lines = remove_same_consecutive(translated_actions + [action])

                executable_mask = True
                nogen_terminate = False
                error_message = None

                obj, skill, loc, precondition_error, error_params, error_msg, skippable = scene_environment.slang_action._parse_step_to_action(None, translated_action)
                ...
                if np.random.random() <= 0.061/(max_steps//2):
            ...
            curr_action = [x.replace('_',' ') for x in curr_action.split(' ')]
            curr_action = fill_action_template(curr_action)
            curr_action = curr_action[0].upper() + curr_action[1:]
